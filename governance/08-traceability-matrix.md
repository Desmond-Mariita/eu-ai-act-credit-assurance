...
- **Tag verification (self-contained):** the repo ships the signer trust anchor at
  ...
  fingerprint). Verify the signed tags in one line:
  `git -c gpg.ssh.allowedSignersFile=.allowed_signers verify-tag prereg-v1 v1.1.2` (all of prereg-v1 /
  v1.0 / v1.1 / v1.1.2 verify; also run by CI). The
...
- **Hashing:** data + metrics artifacts carry SHA256 prefixes in `09`; living governance docs are
  content-addressed by git (blob SHA at tag `v1.1.2`); the pre-registration is OpenTimestamps
