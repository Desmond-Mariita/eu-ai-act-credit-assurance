...
| **FAITH-1** | Movement metric **validated** (clean control at floor); **TreeSHAP +0.106 [.094,.118]**, **LIME +0.082 [.072,.092]** both faithful; TreeSHAP > LIME *under this metric* (ROAR: indistinguishable) | `10 §Results` | EV-009 | `python scripts/30_faithfulness.py` | Art. 86 / GDPR 13-15 |
...
| **FAITH-4** | **ROAR** (8 splits): TreeSHAP 0.108±0.017 ≈ LIME 0.113±0.017 (**indistinguishable**), both ≫ random 0.036 | `10 §ROAR` | EV-015 | `python scripts/70_roar.py` | Art. 86 |
...
- **Hashing:** data + metrics artifacts carry SHA256 prefixes in `09`; living governance docs are
  content-addressed by git (blob SHA at tag `v1.0`); the pre-registration is OpenTimestamps
